...
"out vec4 o_col;\n"
// The real sRGB transfer function, both ways — not the pow(2.2) approximation.
// An sRGB swapchain encodes with the exact curve including its linear toe, so
// decoding with pow(2.2) does not undo it: the two disagree most in the darks,
// which is precisely where a launcher's near-black background lives. With the
// exact pair, decode followed by encode is the identity and the LCD arrives on
// the panel as the bytes the engine drew.
...
"void main() {\n"
"    if (u_mode == 1) {\n"
"        // The LCD, and nothing else. No lift, no boost, no dither: this is a\n"
"        // 128x128 panel being reproduced, and the only correct thing to show\n"
"        // is exactly the pixels the engine drew. An earlier version added a\n"
"        // small emissive lift here so it would read as a lit display — which\n"
"        // put pure black at about 17/255 and washed out every dark game and\n"
"        // the launcher with them. It reads as lit because it is brighter than\n"
"        // the shell around it, which it already was.\n"
"        vec3 c = to_linear(texture(u_tex, v_uv).rgb);\n"
"        o_col = emit(c, 1.0, 0.0);\n"
"    } else if (u_mode == 2) {\n"
...
"        o_col = emit(to_linear(u_colour.rgb), u_colour.a * smoothstep(1.0, 0.25, d), 1.0);\n"
"    } else if (u_mode == 3) {\n"
...
"        o_col = emit(to_linear(u_colour.rgb) * line * fade, u_colour.a * line * fade, 1.0);\n"
"    } else {\n"
...
"        o_col = emit(c * min(0.72 + 0.36 * d, 1.15) * rim, 1.0, 1.0);\n"
"    }\n"
"}\n";
