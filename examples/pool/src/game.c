...
static float s_tip_side, s_tip_vert;/* English / draw-follow, units of R (+-0.45) */
...
static MeshVert tv[256];
static MeshFace tf[200];
static int      tnv, tnf;
static Mesh     table_mesh;
static float    SC;                 /* int8 scale = half_len */

static void pv(float x, float y, float z) {
    tv[tnv].x = (int8_t)(x / SC * 127.0f);
    tv[tnv].y = (int8_t)(y / SC * 127.0f);
    tv[tnv].z = (int8_t)(z / SC * 127.0f);
    tnv++;
}
static void quad(float ax,float ay,float az, float bx,float by,float bz,
    ...
    tf[tnf++] = (MeshFace){i,   i+1, i+2, nx,ny,nz, col};
    tf[tnf++] = (MeshFace){i,   i+2, i+3, nx,ny,nz, col};
}
/* dark pocket disc (octagon fan) on the cloth at (px,pz), radius r */
static void pocket_disc(float px, float pz, float r, uint16_t col) {
    int c = tnv; pv(px, 0.001f, pz);
    for (int k = 0; k < 8; k++) {
        float a = k * 0.7853982f;
        pv(px + r*cosf(a), 0.001f, pz + r*sinf(a));
    }
    for (int k = 0; k < 8; k++)
        tf[tnf++] = (MeshFace){c, c+1+k, c+1+((k+1)%8), 0,127,0, col};
}
...
static void build_table(void) {
    ...
    quad(-hl,0,-hw,  -hl,0,hw,  hl,0,hw,  hl,0,-hw,  0,127,0, cloth);   /* bed */

    float oW = hw + rw, oL = hl + rw;
    /* long rails (along X, at z = +-hw): top + inner face */
    quad(-hl,ch,hw,  hl,ch,hw,  hl,ch,oW,  -hl,ch,oW,   0,127,0, rail);
    quad(-hl,0,hw,   hl,0,hw,   hl,ch,hw,  -hl,ch,hw,   0,0,-127, rails);
    quad(-hl,ch,-oW, hl,ch,-oW, hl,ch,-hw, -hl,ch,-hw,  0,127,0, rail);
    quad(-hl,ch,-hw, hl,ch,-hw, hl,0,-hw,  -hl,0,-hw,   0,0,127, rails);
    /* short rails (along Z, at x = +-hl) */
    quad(hl,ch,-oW,  hl,ch,oW,  oL,ch,oW,  oL,ch,-oW,   0,127,0, rail);
    quad(hl,0,-oW,   hl,0,oW,   hl,ch,oW,  hl,ch,-oW,  -127,0,0, rails);
    quad(-oL,ch,-oW, -oL,ch,oW, -hl,ch,oW, -hl,ch,-oW,  0,127,0, rail);
    quad(-hl,ch,-oW, -hl,ch,oW, -hl,0,oW,  -hl,0,-oW,   127,0,0, rails);

    for (int p = 0; p < s_world.npocket; p++)
        pocket_disc(s_world.pocket[p].x, s_world.pocket[p].z, s_world.pocket_r[p] * 1.25f, hole);

    table_mesh.verts = tv; table_mesh.faces = tf;
    table_mesh.nverts = (uint16_t)tnv; table_mesh.nfaces = (uint16_t)tnf;
    table_mesh.scale = SC; table_mesh.bound_r = SC * 1.7f; table_mesh.lod_lo = 0;
}
...
static void g_init(void) {
    ...
    s_state = AIM; s_aim = 0.0f; s_tip_side = s_tip_vert = 0.0f; s_power = 0.0f;
}
...
static void g_update(float dt) {
    ...
    if (s_state == AIM || s_state == CHARGE) {
        ...
        s_tip_vert = s_tip_vert >  0.45f ?  0.45f : s_tip_vert < -0.45f ? -0.45f : s_tip_vert;
        s_tip_side = s_tip_side >  0.45f ?  0.45f : s_tip_side < -0.45f ? -0.45f : s_tip_side;

        if (mote_pressed(in, MOTE_BTN_A)) { s_state = CHARGE; s_power += 0.7f * dt; if (s_power > 1.0f) s_power = 1.0f; }
        else if (s_state == CHARGE) { if (s_power > 0.02f) strike(); else { s_state = AIM; s_power = 0.0f; } }
    } else { /* SHOOT */
        uint32_t ev = 0;
        ...
        if (!moving) {
            if (!s_balls[0].on) {            /* scratch: respot the cue ball */
                ...
                s_balls[0].orient = m3_identity(); s_balls[0].on = 1; s_balls[0].drop = 0.0f;
            }
            s_state = AIM;
        }
    }
    ...
    for (int i = 0; i < s_n; i++)
        if (s_balls[i].on)
            mote->scene_add_sphere(v3_sub(s_balls[i].pos, cam_pos), s_table.R, ball_color(s_balls[i].id));

    if (s_state != SHOOT && s_balls[0].on) {     /* aim guideline */
        Vec3 dir = v3(cosf(s_aim), 0.0f, sinf(s_aim));
        for (int d = 1; d <= 8; d++) {
            Vec3 p = v3_add(s_balls[0].pos, v3_scale(dir, s_table.R * 2.0f + d * 0.05f));
            mote->scene_add_sphere(v3_sub(p, cam_pos), s_table.R * 0.22f, MOTE_RGB565(250,250,210));
        }
    }
}
