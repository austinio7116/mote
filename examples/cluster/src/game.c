...
static void g_update(float dt){
    ...
    if (mote_pressed(in, MOTE_BTN_UP))   s_countf += CLUSTER_SPLATS_COUNT * 0.7f * dt;
    if (mote_pressed(in, MOTE_BTN_DOWN)) s_countf -= CLUSTER_SPLATS_COUNT * 0.7f * dt;
    if (s_countf > CLUSTER_SPLATS_COUNT) s_countf = CLUSTER_SPLATS_COUNT;
