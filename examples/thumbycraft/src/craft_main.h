...
/* Mote port: set the OS-owned framebuffer the render/HUD calls draw into.
 * Called each frame by the shim (fb arrives per render_band/overlay, not at
 * init, so craft_main_init is passed NULL). */
void craft_main_set_fb(uint16_t *fb);
