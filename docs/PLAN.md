# ThumbyOS + ThumbyEngine — A Native Game-Dev Platform for Thumby Color (RP2350)

> Working names: **ThumbyOS** (the console OS) and **ThumbyEngine** (`libte`, the consolidated engine). Greenfield — a new sibling repo `thumby-platform/`. **No existing project in this workspace is modified.** ThumbyElite / ThumbyCraft / ThumbyCue / ThumbyOne are read-only references we *port from*, not edit.
...
```
            ┌───────────────────────── Host (Linux/Windows) ─────────────────────────┐
            │  thumby CLI  ──►  cross-compile game (arm-none-eabi, -O3)  ──►  game.tgm │
            ...
            └───────────────────────────────────────────────────────────────────────┘
                                              │ USB
            ┌──────────────────────────────── Device: ThumbyOS ────────────────────────┐
            │  Boot ROM → ThumbyOS kernel (flash-resident, low flash region)            │
            ...
            │    • RESIDENT ThumbyEngine (libte) exposed via fixed jump-table ABI       │
...
## 2. The consolidated `ThumbyEngine` (`libte`) core

This is **packaging + de-vendoring**, not a rewrite. Preserve the zero-`#ifdef` `game/` discipline: all platform behind one header (`te_platform.h`), engine compiles identically for host (SDL2) and device (RP2350). Module breakdown and where each comes from:

| Module (`engine/...`) | Header / API | Ported from |
|---|---|---|
| `math/te_vec.h` | `Vec3`, `Mat3` (orthonormal basis), add/sub/scale/dot/cross/lerp/normalize/rotate/orthonormalize, header-only, `-ffast-math` | Elite `vec.h` |
| `render/te_raster.c` | half-space edge-function filled triangles + uint16 depth (LARGER=NEARER) | Elite `r3d_raster.c` |
| `render/te_pipe.c` | transform → near-clip → project → flat-shade | Elite `r3d_pipe.c` |
| `render/te_scene3d.c` | draw-list + **dual-core band split** + volatile go/done handshake | Elite `r3d_scene.c` |
| `render/te_raycast.c` | DDA voxel raycaster path (`TE_RENDER_RAYCAST` flag) | Craft `craft_render.c` |
| `render/te_impostor.c` | per-pixel sphere impostors (planets/balls) | Elite/Cue |
| `render/te_sprite.c` | 2D blit, RGB565 fb ops, uint8 z-occlusion | Craft |
| `render/te_fx.c` | particle/beam FX pool | Elite `r3d_fx.c` |
| `render/te_framebuffer.h` | 128×128 RGB565 fb + depth + band region descriptor | Elite/Craft |
| `physics/te_phys3d.c` | generalized impulse rigid body: pos/vel/avel/orient, ~2kHz substep accumulator, friction phases, CCD, pluggable contact model (billiard cushion/pocket plugs back in) | Cue `cue_physics.c` |
| `audio/te_audio.c` | 22050Hz PWM procedural synth voice-pool + instrument-param table | Elite/Craft |
| `assets/te_mesh.h` | int8 quant verts, 8-byte faces (idx+int8 normal+RGB565), LOD | Elite `r3d_mesh.h` |
| `assets/te_voxel.c` | 64³ sliding window + `te_gen_column`/`te_gen_stamp` hooks | Craft worldgen |
| `assets/te_font.c` | 8×8 bitmap font | Craft `craft_font.c` |
| `assets/te_image.c` | RGB565 texture/atlas decode | new (small) |
| `input/te_input.h` | `TeButtons` (9 bools) + edge/held bookkeeping, platform-independent | Craft `CraftRawButtons` |
| `core/te_platform.h` | the one abstraction: present fb, poll buttons, rumble, micros, VFS, save | Elite `elite_platform.h` |
| `core/te_app.c` | engine lifecycle: init / frame / shutdown; the loop game modules drive | new (thin) |

**Render flags** (`te_config.h`): `TE_RENDER_RASTER`, `TE_RENDER_RAYCAST`, `TE_RENDER_SPRITE`, `TE_PHYSICS`. The resident OS engine is built with **all** backends (universal host). A game declares in its manifest which it uses (lets the launcher validate + lets future native-link builds drop unused code).

`te_platform.h` is implemented twice: `platform/device/` (RP2350 — LCD `gc9107`, buttons, PWM audio, rumble drivers ported from Craft) and `platform/host/` (SDL2, ported from Elite `host/`).
...
- **Stored at arbitrary (but contiguous) physical offsets** in the game-store filesystem. On launch, ThumbyOS programs the RP2350 **ATRANS** address-translation window to map the selected game's physical flash location → the fixed virtual execution window, restores fast QPI XIP, and jumps to the game entry. The game then **executes in place via XIP — no copy to RAM, no relocation, no PIC** (it's linked at the fixed virtual address; ATRANS does the placement).
- **Engine ABI = a versioned jump table** (a `const TeApi *` struct of function pointers) at a fixed known address in the resident region. Games `#include <te_api.h>` and call `te->draw_mesh(...)`, `te->phys_step(...)`, etc. The header declares `extern const TeApi *const te;` resolved by the game linker script to the fixed jump-table address. `TeApi.abi_version` is checked by the launcher before jumping.
...
> **RP2350 gotchas to bake in regardless (learned from ThumbyOne):** restore fast QPI XIP after any flash op or remap (`*_xip_fast_setup` equivalent); place engine hot loops (`te_raster`, `te_raycast`, `te_phys` substep) in IRAM via `.time_critical` (`TE_HOT` macro); bump the tiny Pico default heap via `target_compile_definitions`; the second core must never cross into game-launch/flash ops mid-frame.

---

## 4. Game package / on-disk format

A built game is **one file** `game.tgm` (Thumby Game Module): a small header + the native module image + baked asset blobs, all contiguous so it can be ATRANS-mapped or arena-copied wholesale.

```
game.tgm:
  [header 'TGM1' | abi_version | entry_offset | icon_offset | render_flags | ram_need | name]
  [native .text/.rodata  — linked at fixed virtual window, calls engine via te-> jump table]
  [baked asset blobs: meshes (te_mesh.h), textures (RGB565), audio (PWM params), voxelgen seeds]
```

Source layout on host (only `game.tgm` ships to device):
```
mygame/
...
└─ build/game.tgm     # produced by `thumby build`
```
...
## 5. ThumbyOS (minimal launcher + engine)
...
- **Launcher UI**: scroll the game catalog, render each `icon.bmp`, A = launch. Reuse `te_font` + `te_sprite` for the UI itself (the launcher is the engine's first client).
...
- **USB deploy service**: implements the device side of the custom protocol (§6): receive a `.tgm`, write it contiguously to the store, update catalog, optionally launch immediately. Also streams back logs / a profiler line (frame ms, core balance, heap) for the CLI.
- **Launch path**: ATRANS-map (or arena-copy) selected `.tgm`, set up game RAM region + stack, jump to entry. **Return to launcher**: game calls `te->exit()` → OS reboots/returns to launcher (watchdog-reboot pattern, same family as ThumbyOne handoff but single-target).
...
- **`obj2mesh`** — OBJ → `te_mesh.h` baked mesh. Port from Elite as-is.
- **`img2tex.py`** — PNG/atlas → RGB565 texture blob. New, small.
- **`wav2pwm.py`** — WAV/instrument spec → PWM synth voice params. New.
- **SDL2 host build of `libte`** — same engine + `te_plat_host.c`; games compile *natively for the host too* (the host links the game's `src/*.c` directly against the SDL `libte` and a host shim of the ABI jump table) so `thumby run` gives an instant play-in-emulator loop with the real engine. Port the SDL event-loop / keyboard-mapping from Elite `host/host_main.c`.
- **Cross-compile** — `arm-none-eabi-gcc -O3 -mcpu=cortex-m33 -mfpu=fpv5-sp-d16` against the game linker script (fixed virtual window + RAM region) + `te_api.h`. Produces the native module; `thumby` wraps it + bakes assets → `game.tgm`.
- **`thumby` CLI** (Python) — developer entry point:
  - `thumby new <id>` — scaffold `game.toml` + `src/main.c` + sample asset.
  - `thumby build` — bake assets + cross-compile + emit `game.tgm`.
...
New `thumby-platform/` repo. CMake builds an empty `libte` for host (SDL2) + device (pico-sdk). Stand up `te_platform.h` both ways. "Hello triangle": port `te_vec`/`te_pipe`/`te_raster`, render one spinning mesh on SDL **and** device. No OS, no loader yet. *Deliverable: identical engine code renders on host + device.*

**Phase 1 — MINIMAL FIRST MILESTONE (proves the whole thesis).**
> *ThumbyOS boots to a launcher, and one separately-compiled native game module — pushed over USB from the host — loads and runs on the device, plus runs in the SDL emulator. The game calls the engine only through the ABI jump table.*
- Define `te_api.h` jump table + game linker script (fixed virtual window + RAM region).
...
Wire the rest of the ABI: `te_audio`, `te_phys3d`, `te_fx`, `te_impostor`, `te_sprite`, and the **raycast** path + `te_voxel`. Ship one real sample per renderer: a polygon shooter (raster), a voxel sandbox (raycast), a physics toy (reuse Cue's solver). These double as ABI stress tests + docs.

**Phase 4 — Polish & SDK docs.**
Error/crash overlay on device, host hot-rebuild watch, `te_api.h` reference docs, sample-driven "Getting Started," ABI-version compatibility checks in launcher + CLI.
...
- **ABI stability.** The jump table is a forever-contract. Version it (`TeApi.abi_version`), only append, never reorder; launcher + CLI refuse mismatched `.tgm`. A botched ABI break orphans every shipped game.
...
├─ platform/
│  ├─ device/     # RP2350 te_platform impl + gc9107/buttons/audio/rumble (port from Craft)
│  └─ host/       # SDL2 te_platform impl (port from Elite host/)
├─ os/            # ThumbyOS (§5): boot, launcher, settings, game store, USB service, loader
├─ sdk/
│  ├─ te_api.h    # the ABI jump-table contract (forever-stable)
...
- `sdk/te_api.h` — the ABI; the entire platform's stability hinges on it.
- `os/loader.c` — ATRANS-map / arena-copy + jump; the flagged decision.
- `engine/core/te_platform.h` — host/device parity boundary.
