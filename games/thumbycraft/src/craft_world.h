...
extern uint8_t *craft_world_blocks;
/* Inject the world buffer (CRAFT_WORLD_VOXELS bytes). The Mote shim allocates
 * it from the engine arena at startup; must be called before any world use. */
void craft_world_set_buffer(void *p);
