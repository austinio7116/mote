...
/* Mote port: the 256 KB world buffer is too large for the module's 128 KB
 * static-BSS region, so the shim allocates it from the engine arena and
 * injects it here via craft_world_set_buffer(). Every access below is
 * unchanged — indexing a pointer behaves identically to indexing the array. */
...
/* CRAFT_LIGHT_MAX=3 means light reaches only ~6 cells, so a single source's BFS
 * frontier never exceeds ~150 nodes — 256 is ample. The original 1024-node queue
 * lived on the STACK (a ~7 KB frame), which overflowed the Mote slot runner's 4 KB
 * stack and corrupted memory during the world-load relight (the launch hang). It now
 * comes from the engine arena (allocated once), so the stack frame is tiny. */
...
#define LIGHTSRC_MAX 256        /* Mote: trimmed to fit module RAM; torches in the 64x64 window */
