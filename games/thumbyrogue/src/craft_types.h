...
/* Mote device module: place the hot raycaster in .ramtext, which the loader
 * copies from the flash image to SRAM at load — so the per-pixel DDA inner loop
 * runs from RAM instead of XIP flash and stops contending with the texture
 * atlas for the XIP cache (the single biggest device FPS lever). Affordable
 * because the 64x32x64 world (see cflags) halves the lightmap. Host .so: no-op. */
