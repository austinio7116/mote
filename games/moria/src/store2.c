...
#ifdef MOTE
  moria_ui_mode = MI_CHOICE;   /* native list populated by the caller */
#endif
...
int *cur_top;
{
  ...
  if (*cur_top == 12)
    ...
    i = s_ptr->store_ctr - 1;
#ifdef MOTE
  /* Populate the native picker with the visible stock (name + price). */
  {
    register int k;
    moria_choice_reset("Buy which item?");
    for (k = 0; k <= i && k < MCH_MAX; k++)
      {
	register inven_record *rr = &s_ptr->store_inven[*cur_top + k];
	bigvtype nm; char lbl[MCH_LBL];
	objdes(nm, (inven_type *)&rr->sitem, TRUE);
	if (rr->scost > 0)
	  (void) snprintf(lbl, MCH_LBL, "%.19s %ldau", nm, (long)rr->scost);
	else
	  (void) snprintf(lbl, MCH_LBL, "%.24s", nm);
	moria_choice_add('a' + k, lbl);
      }
  }
#endif
...
int store_num;
{
  ...
  if (s_ptr->store_open < turn)
      ...
      do
	{
	  ...
	  msg_flag = FALSE;
#ifdef MOTE
	  moria_choice_reset("Store");
	  moria_choice_add('p', "Buy");
	  moria_choice_add('s', "Sell");
	  moria_choice_add('w', "Wield/Wear");
	  moria_choice_add('t', "Take off");
	  moria_choice_add('i', "Inventory");
	  moria_choice_add('d', "Drop");
	  if (s_ptr->store_ctr > 12) moria_choice_add('b', "Next page");
	  moria_choice_add(27, "Leave store");
