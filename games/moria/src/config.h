...
/* Mote (Thumby Color) port.  MOTE selects a self-contained screen-manager
   platform (like the historical MAC port): no unix termios/ioctl/curses, no
   signals, no filesystem save files -- the terminal, input, saves and timing
   are all provided by the Mote engine glue (see mote_glue.c / mote_term.c).
   We still define USG so the portable core uses <string.h> and index=strchr
   and the SYS-V style stdio calls that files.c / save.c expect. */
#define MOTE
#ifdef MOTE
#ifndef USG
#define USG
