...
#ifndef VMS
#ifndef MAC
#if !defined(ATARIST_MWC) && !defined(AMIGA) && !defined(MOTE)
long time();
#endif
#endif
#endif
...
#ifndef __TURBOC__
#if (defined(USG) || defined(atarist) || defined(HPUX)) && !defined(VMS) && !defined(MOTE)
...
int show_player;
{
  ...
  high_scores score;
#ifdef MOTE
  /* No scorefile on the handheld (highscore_fp is NULL) -- avoid the NULL
     FILE* dereference that would hard-fault the device. */
  if (highscore_fp == NULL)
    {
      prt("No high scores on this device.", 0, 0);
      pause_line(23);
      return;
    }
#endif
...
void exit_game ()
{
  ...
  if (turn >= 0)
    {
#ifndef MOTE
      /* The 80-col tombstone is replaced by the native game-over screen. */
      if (total_winner)
	kingly();
      print_tomb();
#endif
...
#ifndef MOTE
      highscores();            /* no scorefile on the handheld (fp is NULL) */
      display_scores (TRUE);   /* native game-over screen shows this instead */
#endif
    }
  erase_line (23, 0);
  restore_term ();
  ...
  goback();
#elif defined(MOTE)
  /* Return control to the Mote OS instead of killing the process.  This
     switches back to the engine's fiber and never returns here. */
  mote_game_over();
#else
  exit (0);
#endif
