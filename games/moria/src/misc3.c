...
#ifdef MOTE
/* Mote port: a COMPACT creation summary (fits above the creation menu/keyboard)
   so all six rolled stats are visible for the accept/reroll decision. */
int moria_creation_lines(char lines[][MSHEET_W], int max)
{
  register struct misc *m = &py.misc;
  vtype a, b;
  int n = 0;
#define CRE_ADD(...) do { if (n < max) { \
    (void) snprintf(lines[n], MSHEET_W, __VA_ARGS__); n++; } } while (0)
  CRE_ADD("%s %s", race[m->prace].trace, m->male ? "Male" : "Female");
  CRE_ADD("%s", class[m->pclass].title);
  cnv_stat(py.stats.use_stat[0], a); cnv_stat(py.stats.use_stat[1], b);
  CRE_ADD("STR %-6s INT %s", a, b);
  cnv_stat(py.stats.use_stat[2], a); cnv_stat(py.stats.use_stat[3], b);
  CRE_ADD("WIS %-6s DEX %s", a, b);
  cnv_stat(py.stats.use_stat[4], a); cnv_stat(py.stats.use_stat[5], b);
  CRE_ADD("CON %-6s CHR %s", a, b);
  CRE_ADD("HP %d  AC %d  Au %ld", (int)m->mhp, (int)m->dis_ac, (long)m->au);
#undef CRE_ADD
  return n;
}

/* Mote port: format the character sheet into compact lines the handheld can
   scroll, reusing the same values as put_stats/put_misc*.  */
int moria_sheet_lines(char lines[][MSHEET_W], int max)
{
  register struct misc *m = &py.misc;
  static const char *sl[6] = { "STR", "INT", "WIS", "DEX", "CON", "CHR" };
  vtype b;
  int n = 0, i;
  int xbth, xbthb, xfos, xsrh, xstl, xdis, xsave, xdev;
#define SHEET_ADD(...) do { if (n < max) { \
    (void) snprintf(lines[n], MSHEET_W, __VA_ARGS__); n++; } } while (0)

  SHEET_ADD("Name: %s", m->name);
  SHEET_ADD("%s %s", race[m->prace].trace, m->male ? "Male" : "Female");
  SHEET_ADD("%s", class[m->pclass].title);
  SHEET_ADD("Level %d   Gold %ld", (int)m->lev, (long)m->au);
  SHEET_ADD("Age %d Ht %d Wt %d", (int)m->age, (int)m->ht, (int)m->wt);
  SHEET_ADD(" ");
  for (i = 0; i < 6; i++)
    {
      cnv_stat(py.stats.use_stat[i], b);
      SHEET_ADD("%s : %s", sl[i], b);
    }
  SHEET_ADD(" ");
  SHEET_ADD("HP  %d / %d", (int)m->chp, (int)m->mhp);
  if (m->mana > 0)
    SHEET_ADD("Mana %d / %d", (int)m->cmana, (int)m->mana);
  SHEET_ADD("AC %d   To Hit %+d", (int)m->dis_ac, (int)m->dis_th);
  SHEET_ADD("To Dam %+d  To AC %+d", (int)m->dis_td, (int)m->dis_tac);
  SHEET_ADD("Exp %ld / %ld", (long)m->exp, (long)m->max_exp);
  SHEET_ADD(" ");

  xbth  = m->bth + m->ptohit * BTH_PLUS_ADJ
	+ (class_level_adj[m->pclass][CLA_BTH] * m->lev);
  xbthb = m->bthb + m->ptohit * BTH_PLUS_ADJ
	+ (class_level_adj[m->pclass][CLA_BTHB] * m->lev);
  xfos  = 40 - m->fos; if (xfos < 0) xfos = 0;
  xsrh  = m->srh;
  xstl  = m->stl + 1;
  xdis  = m->disarm + 2 * todis_adj() + stat_adj(A_INT)
	+ (class_level_adj[m->pclass][CLA_DISARM] * m->lev / 3);
  xsave = m->save + stat_adj(A_WIS)
	+ (class_level_adj[m->pclass][CLA_SAVE] * m->lev / 3);
  xdev  = m->save + stat_adj(A_INT)
	+ (class_level_adj[m->pclass][CLA_DEVICE] * m->lev / 3);

  SHEET_ADD("Fighting  : %s", likert(xbth, 12));
  SHEET_ADD("Bows/Throw: %s", likert(xbthb, 12));
  SHEET_ADD("Save      : %s", likert(xsave, 6));
  SHEET_ADD("Stealth   : %s", likert(xstl, 1));
  SHEET_ADD("Disarming : %s", likert(xdis, 8));
  SHEET_ADD("MagicDevic: %s", likert(xdev, 6));
  SHEET_ADD("Perception: %s", likert(xfos, 3));
  SHEET_ADD("Searching : %s", likert(xsrh, 6));
  SHEET_ADD("Infravsion: %d ft", py.flags.see_infra * 10);
#undef SHEET_ADD
  return n;
}
#endif
...
int first_spell;
{
  ...
  *sn = -1;
  ...
  offset = (class[py.misc.pclass].spell==MAGE ? SPELL_OFFSET : PRAYER_OFFSET);
#ifdef MOTE
  /* Drive the spell choice through the native menu (name / mana / fail%). */
  {
    int q;
    moria_choice_reset(prompt);
    for (q = 0; q < num && q < MCH_MAX; q++)
      {
	spell_type *sp = &magic_spell[py.misc.pclass-1][spell[q]];
	char lbl[MCH_LBL];
	(void) snprintf(lbl, MCH_LBL, "%s %dm %d%%",
			spell_names[spell[q]+offset], sp->smana, spell_chance(spell[q]));
	moria_choice_add(spell[q]+'a'-first_spell, lbl);
      }
    moria_ui_mode = MI_CHOICE;
  }
#endif
