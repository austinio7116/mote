...
#else
#ifdef MOTE
extern const treasure_type object_list[MAX_OBJECTS];
#else
...
#ifdef MOTE
/* Mote port: colour indices (MC_*) + the shared draw-colour, used by the core
   (loc_symbol for map tiles, the spell functions for effects) to colourise the
   otherwise-monochrome display.  Renderer maps the index to RGB. */
#include "moria_color.h"
/* Mote port: input-context channel for the controller UI (see moria_ui.h). */
#include "moria_ui.h"
#endif
