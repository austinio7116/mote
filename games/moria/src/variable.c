...
#ifdef MOTE
/* Mote controller-UI input context (see moria_ui.h) -- set by the core's input
   choke points, read by the front-end to pick the right on-screen UI. */
int moria_ui_mode = MI_GENERIC;
int moria_item_lo = 0;
int moria_item_hi = 25;
int moria_item_equip = 0;
int moria_item_full = 0;
char *moria_item_mask = 0;
char *moria_item_prompt = 0;
char *moria_str_buf = 0;    /* MI_STRING: in-progress text (not NUL-terminated) */
int   moria_str_len = 0;    /* MI_STRING: chars typed so far                    */
#endif
