...
#include	<stdlib.h>
#include	<string.h>	/* strncpy -- prototyped for the port  -Mote port- */
...
#if !defined(ATARIST_MWC) && !defined(MAC) && !defined(VMS) && !defined(AMIGA) && !defined(MOTE)
long time();
#endif
struct tm *localtime();
...
int y, x;
{
  ...
  if ((cave_ptr->cptr == 1) && (!find_flag || find_prself))
    {
#ifdef MOTE
      mote_draw_color = MC_WHITE;      /* the player */
#endif
      return '@';
    }
    ...
    {
#ifdef MOTE
      mote_draw_color = MC_VIOLET;     /* hallucination */
#endif
      return randint (95) + 31;
    }
  else if ((cave_ptr->cptr > 1) && (m_list[cave_ptr->cptr].ml))
    {
      int mc = c_list[m_list[cave_ptr->cptr].mptr].cchar;
#ifdef MOTE
      mote_draw_color = moria_monster_color(mc);
#endif
      return (unsigned char)mc;
    }
    ...
    {
#ifdef MOTE
      mote_draw_color = moria_object_color(t_list[cave_ptr->tptr].tval);
#endif
    ...
    {
#ifdef MOTE
      mote_draw_color = MC_LDARK;      /* floor */
#endif
#ifdef MSDOS
      return floorsym;
#else
    ...
    {
#ifdef MOTE
      mote_draw_color = MC_SLATE;      /* granite wall */
#endif
#ifdef MSDOS
      return wallsym;
#else
    ...
    {
#ifdef MOTE
      mote_draw_color = MC_UMBER;      /* mineral vein */
#endif
      return '%';
    }
}
...
#ifdef MOTE
/* Mote port: non-zero while the character is still being created (so the
   front-end draws a native summary instead of Umoria's 80-column sheet). */
int moria_in_creation(void) { return !character_generated; }

/* Mote port: the cause of death (for the native game-over screen). */
const char *moria_death_cause(void) { return died_from; }

/* Mote port: the player's STABLE screen position (terminal row/col where the
   player glyph is drawn).  The renderer centres on this instead of the terminal
   cursor, which Umoria moves all over while drawing (that caused a one-frame
   left-aligned flicker as you walked). */
void moria_player_pos(int *srow, int *scol)
{
  if (srow) *srow = char_row - panel_row_prt;
  if (scol) *scol = char_col - panel_col_prt;
}

/* Mote port: the player's ABSOLUTE dungeon position, and the display glyph for
   any dungeon cell.  The renderer draws a window straight from the dungeon,
   always centred on the player -- so there is no status sidebar, no panel, and
   no scroll jump (the view only shifts one tile per step). */
void moria_player_abs(int *y, int *x)
{
  if (y) *y = char_row;
  if (x) *x = char_col;
}

int moria_map_cell(int y, int x, int *col)
{
  int ch;
  if (y < 0 || y >= MAX_HEIGHT || x < 0 || x >= MAX_WIDTH)
    { if (col) *col = MC_DEFAULT; return ' '; }
  mote_draw_color = MC_DEFAULT;
  ch = loc_symbol(y, x);          /* also sets mote_draw_color */
  if (col) *col = mote_draw_color;
  return ch;
}

/* Mote port: the command char for taking the staircase under the player, or 0.
   Lets the front-end offer a one-button "descend/ascend". */
int moria_stair_under_player(void)
{
  cave_type *c = &cave[char_row][char_col];
  if (c->tptr)
    {
      int tv = t_list[c->tptr].tval;
      if (tv == TV_DOWN_STAIR) return '>';
      if (tv == TV_UP_STAIR)   return '<';
    }
  return 0;
}

/* Mote port: enumerate the items the current get_item() prompt can actually
   select, as (key, name) pairs, so the front-end can draw a native list instead
   of the 80-column terminal.  Handles both the pack (inventory) and the worn
   equipment list, matching get_item's letter mapping. */
int moria_item_enum(MoriaItemRow *rows, int max)
{
  bigvtype tmp;
  int n = 0;
  if (moria_item_equip)
    {
      int k, idx = 0;
      for (k = INVEN_WIELD; k < INVEN_ARRAY_SIZE && n < max; k++)
	{
	  if (inventory[k].tval == TV_NOTHING)
	    continue;
	  rows[n].key = (unsigned char)('a' + idx);
	  objdes(tmp, &inventory[k], TRUE);
	  (void) strncpy(rows[n].name, tmp, sizeof rows[n].name - 1);
    ...
    {
      int k;
      for (k = moria_item_lo; k <= moria_item_hi && n < max; k++)
	{
	  if (k < 0 || k >= INVEN_ARRAY_SIZE)
	    continue;
	  if (moria_item_mask && !moria_item_mask[k])
	    continue;
	  if (inventory[k].tval == TV_NOTHING)
	    continue;
	  rows[n].key = (unsigned char)('a' + k);
	  objdes(tmp, &inventory[k], TRUE);
	  (void) strncpy(rows[n].name, tmp, sizeof rows[n].name - 1);
	  rows[n].name[sizeof rows[n].name - 1] = '\0';
	  n++;
...
/* Mote port: fill the compact HUD vitals from the player record. */
...
/* Mote port: colour a monster by its display letter (Angband-ish scheme) and
   an object by its tval.  These only affect the renderer; game logic is
   unchanged. */
unsigned char moria_monster_color(int cchar)
{
  switch (cchar)
    {
    case 'd': case 'D': return MC_RED;      /* dragons / drakes            */
    case 'u': case 'U': return MC_LRED;     /* demons                      */
    case 'p': case 'h': return MC_LWHITE;   /* humans / humanoids          */
    case 't':           return MC_LUMBER;   /* townspeople                 */
    case 'o':           return MC_UMBER;    /* orcs                        */
    case 'k':           return MC_LGREEN;   /* kobolds                     */
    case 'T': case 'P': return MC_GREEN;    /* trolls / giants             */
    case 'O':           return MC_UMBER;    /* ogres                       */
    case 'g':           return MC_SLATE;    /* golems / ghouls             */
    case 'G': case 'W': return MC_LWHITE;   /* ghosts / wraiths            */
    case 'z': case 'Z': return MC_SLATE;    /* zombies                     */
    case 's':           return MC_WHITE;    /* skeletons                   */
    case 'L': case 'V': return MC_VIOLET;   /* liches / vampires           */
    case 'e':           return MC_VIOLET;   /* floating eyes               */
    case 'w':           return MC_LWHITE;   /* worms                       */
    case 'r':           return MC_LUMBER;   /* rodents                     */
    case 'b':           return MC_UMBER;    /* bats                        */
    case 'c': case 'C': return MC_YELLOW;   /* canines / dogs              */
    case 'f': case 'F': return MC_ORANGE;   /* felines / flies             */
    case 'J': case 'j': return MC_LGREEN;   /* snakes / jellies            */
    case 'm': case 'i': return MC_LGREEN;   /* molds / icky things         */
    case 'n':           return MC_ORANGE;   /* nagas                       */
    case 'q': case 'Q': return MC_LUMBER;   /* quadrupeds                  */
    case 'R':           return MC_LGREEN;   /* reptiles                    */
    case 'S':           return MC_SLATE;    /* spiders                     */
    case 'y': case 'Y': return MC_YELLOW;   /* yeeks / apes                */
    case 'H':           return MC_LRED;     /* hybrid monsters             */
    case 'M':           return MC_LGREEN;   /* hydras                      */
    default:            return MC_LWHITE;
    }
}

unsigned char moria_object_color(int tval)
{
  switch (tval)
    {
    case TV_SLING_AMMO: case TV_BOLT: case TV_ARROW: return MC_LUMBER;
    case TV_BOW:                                     return MC_UMBER;
    case TV_HAFTED: case TV_POLEARM: case TV_SWORD:  return MC_LWHITE;
    case TV_DIGGING:                                 return MC_SLATE;
    case TV_BOOTS: case TV_GLOVES: case TV_CLOAK:
    case TV_HELM: case TV_SHIELD: case TV_HARD_ARMOR:
    case TV_SOFT_ARMOR:                              return MC_SLATE;
    case TV_AMULET:                                  return MC_ORANGE;
    case TV_RING:                                    return MC_LBLUE;
    case TV_STAFF:                                   return MC_LUMBER;
    case TV_WAND:                                    return MC_LBLUE;
    case TV_FLASK:                                   return MC_LRED;
    case TV_SCROLL1: case TV_SCROLL2:                return MC_WHITE;
    case TV_POTION1: case TV_POTION2:                return MC_LRED;
    case TV_FOOD:                                    return MC_LUMBER;
    case TV_MAGIC_BOOK:                              return MC_LBLUE;
    case TV_PRAYER_BOOK:                             return MC_LGREEN;
    case TV_GOLD:                                    return MC_YELLOW;
    case TV_LIGHT:                                   return MC_YELLOW;
    case TV_CHEST:                                   return MC_UMBER;
    default:                                         return MC_LWHITE;
    }
}
#endif
