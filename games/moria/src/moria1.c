...
char *mask;
{
  ...
  if (col < 0)
    col = 0;
#ifdef MOTE
  col = 0;   /* left-align lists so they fit the handheld's narrow viewport */
#endif
	  ...
	  if (from > to)
		...
		disp = "";
	      (void) sprintf(prt1,
		    ...
		    , prompt);
#ifdef MOTE
	      /* Drive this selection through the native item picker. */
	      moria_item_lo = from;
	      moria_item_hi = to;
	      moria_item_equip = (command == 't' || command == 'r');
	      moria_item_full = (swap[0] != '\0');
	      moria_item_mask = 0;
	      moria_item_prompt = prompt;
	      moria_ui_mode = MI_ITEM;
#endif
	  	      /* Abort everything. */
  ...
  *com_val = 0;
  ...
  if (inven_ctr > 0 || (full && equip_ctr > 0))
      ...
      do
	{
	  ...
	  prt(out_val, 0, 0);
#ifdef MOTE
	  /* Tell the controller UI the current selectable letter range so it
	     can drive a cursor over the item list instead of typing letters. */
	  moria_item_lo = i;
	  moria_item_hi = j;
	  moria_item_equip = (i_scr <= 0);
	  moria_item_full = full;
	  moria_item_mask = mask;
	  moria_item_prompt = pmt;
	  moria_ui_mode = MI_ITEM;
#endif
	  do
	    {
