...
#else
/* MOTE: const so the 16 KB template table lives in flash (.rodata), not the
   134 KB game RAM.  It is read-only in practice (the only writer, price_adjust,
   is compiled out at COST_ADJ==100). */
#ifdef MOTE
const
#endif
