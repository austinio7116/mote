...
static void choose_race()
{
  ...
  while (j < MAX_RACES);
#ifdef MOTE
  moria_choice_reset("Choose a race");
  { int q; for (q = 0; q < MAX_RACES; q++) moria_choice_add('a' + q, race[q].trace); }
...
#ifdef MOTE
  moria_choice_reset("Choose a sex");
  moria_choice_add('m', "Male");
  moria_choice_add('f', "Female");
  moria_ui_mode = MI_CHOICE;
#endif
  do
    {
...
static void get_class()
{
  ...
  while (j < MAX_CLASS);
#ifdef MOTE
  moria_choice_reset("Choose a class");
  { int q; for (q = 0; q < k; q++) moria_choice_add('a' + q, class[cl[q]].title); }
  moria_ui_mode = MI_CHOICE;
#endif
  py.misc.pclass = 0;
  exit_flag = FALSE;
...
#ifdef MOTE
  moria_choice_reset("Characteristics");
  moria_choice_add(ESCAPE, "Accept");
  moria_choice_add(' ', "Reroll");
  moria_ui_mode = MI_CHOICE;
#endif
  do
    {
