...
#endif
#else /* not msdos */
#if !defined(ATARI_ST) && !defined(MAC) && !defined(AMIGA) && !defined(MOTE)
#ifndef VMS
#include <sys/ioctl.h>
#endif
#include <signal.h>
...
#if !defined(MAC) && !defined(MSDOS) && !defined(ATARI_ST) && !defined(VMS) && !defined(MOTE)
#ifndef AMIGA
#ifdef USG
#ifdef __linux__
...
#elif defined(MOTE)
{
  /* The Mote engine owns the framebuffer; mote_term_init() sets up the
     virtual 80x24 screen that the curses shim writes into.  savescr is the
     spare window used by save_screen()/restore_screen(). */
  mote_term_init();
...
/* Nothing to do -- the Mote engine owns the display */
{
}
#else
{
...
#ifdef MOTE
{
  msg_print("Not available on the Thumby Color.");
}
#elif defined(atarist) && defined(__GNUC__)
...
#ifdef MOTE
  mote_draw_color = MC_DEFAULT;   /* don't let a glyph colour bleed onward */
#endif
}
#endif
...
int row, column, slen;
{
  ...
  p = in_str;
#ifdef MOTE
  moria_ui_mode = MI_STRING;
  moria_str_buf = in_str;
  moria_str_len = 0;
#endif
  do
    {
#ifdef MOTE
      moria_str_len = (int)(p - in_str);   /* publish in-progress text */
#endif
      i = inkey();
      switch(i)
	{
