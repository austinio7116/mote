...
#ifdef MOTE
/* Mote port: the save file is a blob in the engine arena (mote->kv_save), not a
   real file.  ms_put/ms_get replace putc/getc on the buffer throughout the
   wr and rd primitives; _save_char/get_char swap fopen/fclose for kv_put/kv_get. */
#define MOTE_SAVE_KEY "moria.sav"
static unsigned char *ms_buf;
static int ms_pos, ms_cap, ms_len;
#define ms_put() do { if (ms_buf && ms_pos < ms_cap) ms_buf[ms_pos] = xor_byte; ms_pos++; } while (0)
#define ms_get() (ms_pos < ms_len ? ms_buf[ms_pos++] : 0)
#else
#define ms_put() ((void)putc((int)xor_byte, fileptr))
#define ms_get() (getc(fileptr) & 0xFF)
#endif
...
static int sv_write()
{
  ...
  if (death)
    {
#ifdef MOTE
      return TRUE;   /* buffered; kv_put checks overflow in _save_char */
#else
...
char *fnam;
{
  ...
  ok = FALSE;
#ifdef MOTE
  /* Serialise into the arena save buffer; kv_put it below. */
  ms_buf = mote_save_buf(&ms_cap);
  ms_pos = 0;
  fileptr = (FILE *)1;		/* non-null marker so the write block runs */
#else
...
#ifdef MOTE
      if (ms_pos > ms_cap || mote_kv_put(MOTE_SAVE_KEY, ms_buf, ms_pos) < 0)
	ok = FALSE;
      fileptr = NULL;
#else
...
#ifdef MOTE
  else if ((ms_buf = mote_save_buf(&ms_cap)) == 0 ||
	   (ms_len = mote_kv_get(MOTE_SAVE_KEY, ms_buf, ms_cap)) <= 0)
    msg_print("No saved game.");
#elif defined(MAC) || defined(AMIGA)
...
#ifdef MOTE
      ms_pos = 0;
      fileptr = (FILE *)1;	/* non-null marker so the read block runs */
#else
      (void) close(fd);
...
#ifdef MOTE
      c = (ms_pos < ms_len) ? ms_buf[ms_pos++] : EOF;   /* raw EOF peek */
...
#ifdef MOTE
      ms_pos--;			/* put the peeked byte back */
#else
...
#ifdef MOTE
      if (ms_pos > ms_len)	/* read past end => truncated/corrupt */
	goto error;
#else
      if (ferror(fileptr))
	goto error;
