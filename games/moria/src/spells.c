...
#ifdef MOTE
/* Mote port: colour a spell bolt/ball by its element so effects read on the
   colour screen.  Renderer-only; damage logic is unchanged. */
unsigned char moria_gf_color(int typ)
{
  switch (typ)
    {
    case GF_MAGIC_MISSILE: return MC_LWHITE;
    case GF_LIGHTNING:     return MC_YELLOW;
    case GF_POISON_GAS:    return MC_LGREEN;
    case GF_ACID:          return MC_GREEN;
    case GF_FROST:         return MC_LBLUE;
    case GF_FIRE:          return MC_LRED;
    case GF_HOLY_ORB:      return MC_YELLOW;
    default:               return MC_WHITE;
    }
}
#define MORIA_SET_BOLT_COLOR() (mote_draw_color = moria_gf_color(typ))
#else
#define MORIA_SET_BOLT_COLOR() ((void)0)
#endif
  ...
  do
      ...
      else
	{
	  if (c_ptr->cptr > 1)
	    ...
	    {
	      MORIA_SET_BOLT_COLOR(); print('*', y, x);
	      /* show the bolt */
	      put_qio();
	    }
	}
		  ...
		  if (in_bounds(i, j) && (distance(y, x, i, j) <= max_dis)
		      ...
		      if (c_ptr->fval <= MAX_OPEN_SPACE)
			{
			  if (c_ptr->cptr > 1)
			    ...
			    MORIA_SET_BOLT_COLOR(); print('*', i, j);
			}
		    }
