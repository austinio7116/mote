...
/* ============================================================
 * VENDORED from ThumbyGolf src/golf_terrain.c — multi-octave,
 * style-aware value noise. Do not change the math.
 * ============================================================ */
...
/* Lattice value in [-1, 1) at integer grid point (gx, gz). */
static float lattice(int gx, int gz, uint32_t s) {
    return (float)hash32(gx, gz, s) * (1.0f / 2147483648.0f);
}
...
/* Value noise at world (wx, wz) with period p. */
static float noise2d(float wx, float wz, float p, uint32_t s) {
    float u = wx / p, v = wz / p;
    int x0 = (int)floorf(u), z0 = (int)floorf(v);
    float tx = smoothstep(u - x0), tz = smoothstep(v - z0);

    float a = lattice(x0, z0, s) + (lattice(x0 + 1, z0, s) - lattice(x0, z0, s)) * tx;
    float b = lattice(x0, z0 + 1, s) + (lattice(x0 + 1, z0 + 1, s) - lattice(x0, z0 + 1, s)) * tx;
    return a + (b - a) * tz;
}

/* Anisotropic value noise: rotate by (ca, sa) and stretch the v axis by st. */
static float noise_aniso(float wx, float wz, float p, float ca, float sa, float st, uint32_t s) {
    float u = (ca * wx + sa * wz) / p;
    float v = (-sa * wx + ca * wz) / (p * st);
    int x0 = (int)floorf(u), z0 = (int)floorf(v);
    float tx = smoothstep(u - x0), tz = smoothstep(v - z0);
...
/* Per-style octave amplitudes/periods (hero, macro, intermediate, low) + shaping. */
typedef struct {
    float hero_period, hero_amp;
    float macro_period, macro_amp;
    float inter_period, inter_amp;
    float low_period, low_amp;
    float aniso, ridge;
} StyleNoise;
...
/* Full natural terrain height (all octaves) — the rough surface. */
static float natural_h(uint32_t seed, int style, float x, float z) {
    const StyleNoise *s = &STY[style];
    float ca = 0.7071f, sa = 0.7071f, h = BASE_Y;

    float hero = (s->aniso > 1.01f) ? noise_aniso(x, z, s->hero_period, ca, sa, s->aniso, seed)
                                    : noise2d(x, z, s->hero_period, seed);
    if (s->ridge > 0.01f)
        hero = hero * (1.0f - s->ridge) + (1.0f - fabsf(hero)) * s->ridge;

    h += hero * s->hero_amp;
    h += noise2d(x, z, s->macro_period, seed ^ 0x1111u) * s->macro_amp;
    h += noise2d(x, z, s->inter_period, seed ^ 0x2222u) * s->inter_amp;
    h += noise2d(x, z, s->low_period, seed ^ 0x3333u) * s->low_amp;
    return h;
}
...
static float smooth_h(uint32_t seed, int style, float x, float z) {
    ...
    h += hero * s->hero_amp;
    h += noise2d(x, z, s->macro_period, seed ^ 0x1111u) * (s->macro_amp * 0.6f);
    return h;
}
/* ---- end vendored ---- */
...
void golf_generate(GolfHole *h, uint32_t seed) {
    ...
    float ax = h->cup_x - h->bend_x, az = h->cup_z - h->bend_z;
    ...
    h->n_bunker = 0;
    h->bunker_x[0] = h->cup_x + px * 9.0f - ax * 3.0f;
    h->bunker_z[0] = h->cup_z + pz * 9.0f - az * 3.0f;
    h->bunker_r[0] = 4.5f;
    h->bunker_x[1] = h->cup_x - px * 8.0f - ax * 2.0f;
    h->bunker_z[1] = h->cup_z - pz * 8.0f - az * 2.0f;
    h->bunker_r[1] = 4.0f;
    h->bunker_x[2] = h->bend_x + px * 7.0f;
    h->bunker_z[2] = h->bend_z + pz * 7.0f;
    h->bunker_r[2] = 5.0f;
    h->n_bunker = 3;
}
...
static float dist_seg(float px, float pz, float ax, float az, float bx, float bz) {
    ...
    if (l2 < 1e-6f)
        return sqrtf((px - ax) * (px - ax) + (pz - az) * (pz - az));
    float t = ((px - ax) * dx + (pz - az) * dz) / l2;
    if (t < 0) t = 0;
    if (t > 1) t = 1;
    float cx = ax + t * dx, cz = az + t * dz;
    return sqrtf((px - cx) * (px - cx) + (pz - cz) * (pz - cz));
}

/* Distance to the tee->bend->cup centreline (the nearer of the two legs). */
...
float golf_surface(const GolfHole *h, float x, float z) {
    ...
    /* fairway: mown to the low-freq contour within FAIR_HALF, blended out 4m past */
    float rd = golf_route_dist(h, x, z);
    float fw = (rd < FAIR_HALF) ? 1.0f
             : (rd < FAIR_HALF + 4.0f ? smoothstep(1.0f - (rd - FAIR_HALF) * 0.25f) : 0.0f);
    if (fw > 0.0f) ht = nat * (1.0f - fw) + smo * fw;

    /* green: flat at cup height within GREEN_R, blended out 2.5m past */
    float dg = sqrtf((x - h->cup_x) * (x - h->cup_x) + (z - h->cup_z) * (z - h->cup_z));
    float gw = (dg < GREEN_R) ? 1.0f
             : (dg < GREEN_R + 2.5f ? smoothstep(1.0f - (dg - GREEN_R) * 0.4f) : 0.0f);
    if (gw > 0.0f) ht = ht * (1.0f - gw) + h->cup_h * gw;

    /* tee: flat at tee height within TEE_R, blended out 1.5m past */
    float dt = sqrtf((x - h->tee_x) * (x - h->tee_x) + (z - h->tee_z) * (z - h->tee_z));
    float tw = (dt < TEE_R) ? 1.0f
             : (dt < TEE_R + 1.5f ? smoothstep(1.0f - (dt - TEE_R) * 0.66f) : 0.0f);
    ...
    for (int i = 0; i < h->n_bunker; i++) {
        ...
        if (d < h->bunker_r[i])
            ht -= 1.3f * smoothstep(1.0f - d / h->bunker_r[i]);
    }
    return ht;
}
...
int golf_tree(const GolfHole *h, float x, float z) {
    ...
    float cluster = noise2d(x, z, 20.0f, h->seed ^ 0xABCDu) * 0.5f + 0.5f;     /* low-freq cluster */
    float dens = (h->style == GOLF_PARKLAND) ? 0.80f
               : (h->style == GOLF_HEATHLAND ? 0.60f : 0.45f);
