...
| ~~**3. Life**~~ | **DONE.** One `Unit` for villagers, kings, deer and wolves; seven drives; the utility brain on a 1/8 stagger with hysteresis (and an immediate re-think for the idle, which is what got 151 of 222 souls off the square); a 16×14 bucket grid for proximity; ageing, births, inherited traits with mutation; twenty species; regrowth and burn recovery so a ruined world heals. Four **pastimes** for the fed and unemployed, **meat** from any plant-eating beast, **husbandry** with a flock a town keeps and culls, **fishing** from a quay, **refugees** who walk to the nearest surviving town, soldiers who **guard** their lord and drive off wolves at arm's reach, and priests who keep the temple. Ceiling raised to 768 units — 445 souls — with the ecology budget made absolute so it no longer scales with the array. |
...
| ~~**5. Kingdoms**~~ | **DONE.** Kings, the diplomacy score, war and peace, mustering, loyalty and secession with the rebel taking its own banner (and its parent's knowledge — a province that secedes does not forget how to make tools). War has **reach** rather than requiring a touching border, and a **motive**: a crown short of iron beside a crown with a surplus covets it. A war can now **take a town** — same buildings, same people, new flag, loyalty 15 — and **alliances** form over a common enemy, which nothing in the game could do before because `ally_with` was only ever cleared. Caravans **trade across quiet borders** for gold. Measured: 22–59 towns taken and 25–38 rebellions per 400-year world. |
| ~~**6. Chronicle**~~ | **DONE.** 160-event ring, three syllable name tables, 14 templates, headline toasts, Follow History, the Chronicle screen as a panel view you can travel from, and grudges read back into the king brain. A discovery and a succession are their own events (both used to borrow the wrong type: `tools` and `the hamlet begins`). |
...
| ~~**10. Audit & polish**~~ | **DONE.** `authoring/audit.sh` (8 worlds × 400 years, 7 invariants), 15 SFX recipes, save/load through `kv_*` as one blob per section (it was 46 KB going into a 39 KB buffer), the tint LUT that took the God's Eye pass back from 83 µs to 45, `authoring/uifit.py` for screen text, and the **gate census** (`MOTEBOX_GATES=1`) — 26 conditions counted, because nine features in this game did nothing for want of a condition that never became true. |


**Released 1.0.0** to the gallery: `docs/motebox-guide.html` is the player's handbook,
`docs/games/Motebox.mote` the handheld binary, with an arm64 module for the Android shell.
