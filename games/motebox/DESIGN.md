...
|---|---|
| "They are not blob47 — just rubbish squares" | **The real fault.** Both earlier attempts were *fills*: one tile repeated over an area with no idea what was next to it, so every biome boundary was a hard pixel step. A fill cannot look like terrain at any level of texture polish, because terrain is mostly EDGES. Every biome is now a real 47-cell blob47 set with a drawn rim and picked-out inner corners, importing the mask→cell contract from roguemote's `blob47.py` rather than reimplementing it. |
