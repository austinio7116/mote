...
rounded body with a lit top bevel, a shaded underside and a drop shadow — and the
new `LB/RB` setting picks where: `TOP` puts them in the window's top corners
where index fingers rest when the phone is held like a gamepad, and `ON SHELL`
puts them on the chassis at the angles its own top edges run at (-23.2 and
+17.4 degrees, fitted to the alpha edge of the product photo).
