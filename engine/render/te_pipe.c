/*
 * ThumbyEngine — 3D geometry pipeline. (Ported from ThumbyElite r3d_pipe.c.)
 *
 ...
 * project, and emit final screen triangles to the frame draw-list (te_scene).
 *
 * Projection works in LOGICAL 128x128 space (centre = 64, focal = 64/tan).
 * te_scene scales by TE_SS before rasterising.
 */
#include "te_pipe.h"
#include <math.h>
...
static Vec3    s_view[TE_MAX_VERTS];
static float   s_sx[TE_MAX_VERTS], s_sy[TE_MAX_VERTS];
static uint16_t s_sd[TE_MAX_VERTS];
static uint8_t  s_front[TE_MAX_VERTS];

void te_pipe_set_camera(const Mat3 *cam_basis, float fov_deg) {
    s_cam = *cam_basis;
    s_focal = (TE_FB_W * 0.5f) / tanf(fov_deg * (3.14159265f / 180.0f) * 0.5f);
...
static inline void project(Vec3 v, float *sx, float *sy, uint16_t *sd) {
    ...
    *sx = (TE_FB_W * 0.5f) + s_focal * v.x * inv_z;
    *sy = (TE_FB_H * 0.5f) - s_focal * v.y * inv_z;
    float d = TE_DEPTH_K * inv_z;
...
const Mat3 *te_pipe_camera(void) { return &s_cam; }
float te_pipe_focal(void) { return s_focal; }

int te_pipe_draw_object(const TeObject *obj) {
    return te_pipe_draw_object_scaled(obj, 1.0f);
}

int te_pipe_draw_object_scaled(const TeObject *obj, float os) {
    const Mesh *mesh = obj->mesh;
    if (!mesh || mesh->nverts > TE_MAX_VERTS) return 0;
    ...
    if (c_view.z + br < TE_NEAR) return 0;
    ...
    for (int f = 0; f < mesh->nfaces; f++) {
        ...
        if (in_count == 3) {
            te_emit_tri(s_sx[a], s_sy[a], s_sd[a],
        ...
        for (int i = 0; i < 3; i++) {
            ...
            int p_in = p.z > TE_NEAR, q_in = q.z > TE_NEAR;
            if (p_in) out[n_out++] = p;
            if (p_in != q_in) {
                float tt = (TE_NEAR - p.z) / (q.z - p.z);
        ...
        for (int i = 0; i < n_out; i++) {
            Vec3 v = out[i];
            if (v.z < TE_NEAR * 1.0001f) v.z = TE_NEAR * 1.0001f;
            project(v, &ox[i], &oy[i], &od[i]);
        }
        te_emit_tri(ox[0], oy[0], od[0], ox[1], oy[1], od[1],
                    ox[2], oy[2], od[2], col);
        drawn++;
        if (n_out == 4) {
            te_emit_tri(ox[0], oy[0], od[0], ox[2], oy[2], od[2],
                        ox[3], oy[3], od[3], col);
            drawn++;
        }
    }
