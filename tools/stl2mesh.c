...
/* Triplanar UV for vert `v` given face normal `n`, extent `ext` (recentred coord
 * range). Picks the plane perpendicular to the dominant normal axis, maps the
 * other two coords to 0..255. v is flipped (255-..) to match the engine's
 * top-left tpix[v*w+u] sampling. */
static void triplanar_uv(V3 v, V3 n, float ext, int *ub, int *vb){
    float ax=fabsf(n.x), ay=fabsf(n.y), az=fabsf(n.z);
    float fu, fv;
    if(ax>=ay && ax>=az){ fu=v.z; fv=v.y; }        /* X dominant -> ZY plane */
    else if(ay>=ax && ay>=az){ fu=v.x; fv=v.z; }   /* Y dominant -> XZ plane */
    else { fu=v.x; fv=v.y; }                        /* Z dominant -> XY plane */
    float u=(fu/ext)+0.5f, w=(fv/ext)+0.5f;         /* -> 0..1 */
    int iu=(int)lrintf(u*255.0f), iv=255-(int)lrintf(w*255.0f);
    if(iu<0)iu=0; if(iu>255)iu=255; if(iv<0)iv=0; if(iv>255)iv=255;
    *ub=iu; *vb=iv;
}
...
int main(int argc,char**argv){
    ...
    /* Optional texture: a PNG next to the .stl with the same basename
     * (model.stl -> model.png). If present, embed it and generate triplanar
     * per-face UVs (project each face onto the plane perpendicular to its
     * dominant normal axis, normalised to the model extent). */
    int textured=0, tex_avg=0; { char png[600]; size_t il=strlen(in);
        const char *dot=strrchr(in,'.'); size_t base=dot?(size_t)(dot-in):il;
        snprintf(png,sizeof png,"%.*s.png",(int)base,in);
        FILE *pf=fopen(png,"rb");                 /* probe quietly — no sidecar is normal */
        if(pf){ fclose(pf); int tw,th; if(tex_embed(h,name,png,&tw,&th,&tex_avg)){ textured=1; fprintf(h,"\n"); } } }
    /* triplanar extent (recentred space): verts span ~[-maxc,+maxc] on each axis */
    float tp_ext = 2.0f*maxc; if(tp_ext<1e-6f) tp_ext=1.0f;
    ...
    while(ti<ONF){
        ...
        fprintf(h,"};\n");
        if(textured){   /* triplanar per-face UVs (u0,v0,u1,v1,u2,v2) */
            fprintf(h,"static const uint8_t %s_uv%d[%d]={\n",name,chunk,cf*6);
            for(int i=0;i<cf;i++){
                V3 a=OV[cv[cface[i][0]]], b=OV[cv[cface[i][1]]], c=OV[cv[cface[i][2]]];
                V3 n=v3cross(v3sub(b,a),v3sub(c,a)); float l=v3len(n);
                if(l<1e-9f){ n=(V3){0,0,1}; l=1; } n.x/=l; n.y/=l; n.z/=l;
                int ua,va,ubb,vbb,uc,vc;
                triplanar_uv(a,n,tp_ext,&ua,&va); triplanar_uv(b,n,tp_ext,&ubb,&vbb); triplanar_uv(c,n,tp_ext,&uc,&vc);
                fprintf(h,"%d,%d,%d,%d,%d,%d,\n",ua,va,ubb,vbb,uc,vc);
            }
            fprintf(h,"};\n");
        }
        if(textured)
            cl+=snprintf(chunklist+cl,sizeof(chunklist)-cl,
                "  {%s_v%d,%s_f%d,0,%d,%d,0x%04X,%.6ff,%.6ff,0,&%s_tex,%s_uv%d},\n",
                name,chunk,name,chunk,nv,cf,tex_avg,maxc,bound_r,name,name,chunk);
        else
