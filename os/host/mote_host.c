/*
 * Mote OS — host entry. The PC analogue of the device OS: instead of mapping
 * a .mote out of flash, it dlopen()s a game module .so, checks its ABI version,
 * hands it the engine jump table, and runs the resident frame loop.
 *
 * This proves the platform thesis on the host: a separately-built native game
 * module, loaded at runtime, drives the resident engine purely through the
 * ABI — exactly what the device loader will do, minus the flash mechanics.
 *
 *   mote_host <game.so>
 */
#include "mote_os.h"
#include "mote_platform.h"
#include <dlfcn.h>
#include <stdio.h>

int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: %s <game.so>\n", argv[0]);
        return 2;
    }
    const char *path = argv[1];

    void *mod = dlopen(path, RTLD_NOW | RTLD_LOCAL);
    if (!mod) {
        fprintf(stderr, "mote_os: dlopen failed: %s\n", dlerror());
        return 1;
    }

    const uint32_t *game_abi = (const uint32_t *)dlsym(mod, "mote_game_abi_version");
    MoteGameRegisterFn reg = (MoteGameRegisterFn)dlsym(mod, "mote_game_register");
    if (!game_abi || !reg) {
        fprintf(stderr, "mote_os: '%s' is not a game module "
                        "(missing mote_game_abi_version / mote_game_register)\n", path);
        dlclose(mod);
        return 1;
    }
    if (*game_abi > MOTE_ABI_VERSION) {
        fprintf(stderr, "mote_os: game ABI v%u newer than engine v%u — refusing\n",
                *game_abi, MOTE_ABI_VERSION);
        dlclose(mod);
        return 1;
    }

    if (mote_plat_init("Mote OS (host)") != 0) {
        dlclose(mod);
        return 1;
    }

    MoteApi api;
    mote_api_fill(&api);
    const MoteGameVtbl *vt = reg(&api);
    if (!vt) {
        fprintf(stderr, "mote_os: game register returned no vtable\n");
        mote_plat_shutdown();
        dlclose(mod);
        return 1;
    }

    printf("mote_os: loaded '%s' (game ABI v%u, engine v%u)\n",
           path, *game_abi, MOTE_ABI_VERSION);
    mote_os_run(&api, vt);

    mote_plat_shutdown();
    dlclose(mod);
    return 0;
}
