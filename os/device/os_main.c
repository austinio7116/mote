 ...
 * of os/host/mote_host.c (which dlopens a .so) — same engine, same ABI,
 * same os/mote_os loop; only the loader differs.
 *
...
int main(void) {
    ...
    uint32_t map_us = 0;
    const MoteGameVtbl *vt = mote_loader_map_embedded(&api, &map_us);
    if (!vt) {
        show_solid(MOTE_RGB565(180, 0, 0));   /* red: bad/incompatible module */
        while (1) tight_loop_contents();
    }

    mote_os_run(&api, vt);            /* returns only on exit_to_launcher */

    /* No launcher yet — park. */
    show_solid(MOTE_RGB565(0, 0, 60));
    while (1) tight_loop_contents();
    return 0;
}
