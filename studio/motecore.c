...
static int bake_image(const char *path, const char *header, const char *name, mote_log_fn log){
    ...
    uint16_t *px=malloc((size_t)w*h*2); int keyed=0;
    for(int i=0;i<w*h;i++){ int r=d[i*4],g=d[i*4+1],b=d[i*4+2],a=d[i*4+3]; unsigned v;
        ...
        px[i]=(uint16_t)v; }
    static int cmap[65536]; for(int i=0;i<65536;i++)cmap[i]=-1; uint16_t pal[256]; int npal=0;   /* count unique colours */
    for(int i=0;i<w*h;i++){ if(cmap[px[i]]<0){ if(npal<256)pal[npal]=px[i]; cmap[px[i]]=npal; npal++; } }
    if(npal<=256){   /* lossless palette-indexed: 4bpp (<=16 colours) or 8bpp — 1/4 or 1/2 the flash */
        int fmt=npal<=16?1:2;
        fprintf(f,"static const uint16_t %s_pal[%d] = {",name,npal); for(int i=0;i<npal;i++)fprintf(f,"0x%04x,",pal[i]); fprintf(f,"};\n");
        if(fmt==1){ int nby=(w*h+1)/2; fprintf(f,"static const uint8_t %s_idx[%d] = {\n",name,nby);
            for(int b=0;b<nby;b++){ int i0=b*2,i1=i0+1,hi=cmap[px[i0]],lo=(i1<w*h)?cmap[px[i1]]:0; fprintf(f,"%d,",(hi<<4)|lo); if((b&31)==31)fputc('\n',f); } fprintf(f,"\n};\n"); }
        else { fprintf(f,"static const uint8_t %s_idx[%d] = {\n",name,w*h); for(int i=0;i<w*h;i++){ fprintf(f,"%d,",cmap[px[i]]); if((i&31)==31)fputc('\n',f); } fprintf(f,"\n};\n"); }
        fprintf(f,"static const MoteImage %s_img = { 0, %d, %d, 0x%04X, %d, %d, %s_idx, %s_pal };  /* %dbpp indexed, %d colours */\n#define %s_W %d\n#define %s_H %d\n",name,w,h,0xF81F,keyed?0:1,fmt,name,name,fmt==1?4:8,npal,name,w,name,h);
    } else {   /* >256 colours: plain RGB565 */
        fprintf(f,"static const uint16_t %s_px[%d] = {\n",name,w*h);
        for(int i=0;i<w*h;i++){ fprintf(f,"0x%04x,",px[i]); if((i&15)==15)fputc('\n',f); }
...
/* wav2snd — any PCM WAV (8/16/24/32-bit, mono/stereo, any rate) -> a MoteSound
 * header, kept at the WAV's NATIVE quality: downmix to mono, keep 8/16-bit depth
 * and the source rate (only downsampled if it exceeds the 22050 mixer rate). The
 * engine resamples + expands on playback, so an 8-bit/11025 clip is ~1/4 the flash. */
#define MOTE_MIX_RATE 22050
static int bake_wav(const char *path, const char *header, const char *name, mote_log_fn log){
    ...
    unsigned fmt=1,ch=1,rate=MOTE_MIX_RATE,bits=16;
    long off=-1,len=0,i=12;                                  /* walk RIFF sub-chunks */
    while(i+8<=sz){ unsigned cl=(unsigned)b[i+4]|((unsigned)b[i+5]<<8)|((unsigned)b[i+6]<<16)|((unsigned)b[i+7]<<24);
        if(b[i]=='f'&&b[i+1]=='m'&&b[i+2]=='t'&&b[i+3]==' '&&i+8+16<=sz){
            fmt=(unsigned)b[i+8]|((unsigned)b[i+9]<<8); ch=(unsigned)b[i+10]|((unsigned)b[i+11]<<8);
            rate=(unsigned)b[i+12]|((unsigned)b[i+13]<<8)|((unsigned)b[i+14]<<16)|((unsigned)b[i+15]<<24);
            bits=(unsigned)b[i+22]|((unsigned)b[i+23]<<8); }
        else if(b[i]=='d'&&b[i+1]=='a'&&b[i+2]=='t'&&b[i+3]=='a'){ off=i+8; len=(long)cl; }
        i+=8+(long)cl+((long)cl&1); }
    if(off<0){ off=44; len=sz-44; } if(off+len>sz)len=sz-off; if(ch<1)ch=1; (void)fmt;
    unsigned char *raw=b+off; unsigned out_bits=(bits==8)?8:16;
    /* --- decode to a mono int array (in the OUTPUT depth's range) at the SOURCE rate --- */
    long sn=0; int *mono=NULL;
    if(bits==8){ sn=len/(long)ch; mono=malloc((size_t)(sn>0?sn:1)*sizeof(int));
        for(long fr=0;fr<sn;fr++){ int s=0; long bo=fr*(long)ch; for(unsigned c=0;c<ch;c++) s+=raw[bo+c];
            int v=(s/(int)ch)-128; if(v<-128)v=-128; if(v>127)v=127; mono[fr]=v; } }
    else if(bits==16){ sn=len/2/(long)ch; mono=malloc((size_t)(sn>0?sn:1)*sizeof(int));
        const int16_t *p=(const int16_t*)raw;
        for(long fr=0;fr<sn;fr++){ int s=0; long bo=fr*(long)ch; for(unsigned c=0;c<ch;c++) s+=p[bo+c]; mono[fr]=s/(int)ch; } }
    else { unsigned bps=bits/8; sn=len/(long)(bps*ch); mono=malloc((size_t)(sn>0?sn:1)*sizeof(int));
        for(long fr=0;fr<sn;fr++){ int s=0; long bo=fr*(long)(bps*ch);
            for(unsigned c=0;c<ch;c++){ long o2=bo+(long)(c*bps)+(long)bps-2; s+=(int16_t)((unsigned)raw[o2]|((unsigned)raw[o2+1]<<8)); }
            mono[fr]=s/(int)ch; } }
    /* --- only DOWNsample if the source is faster than the mixer --- */
    int *pcm=mono; long N=sn;
    if(rate>MOTE_MIX_RATE && sn>1){ N=(long)((long long)sn*MOTE_MIX_RATE/(long long)rate);
        pcm=malloc((size_t)(N>0?N:1)*sizeof(int)); double step=(double)rate/MOTE_MIX_RATE,pos=0.0;
        for(long j=0;j<N;j++){ long k=(long)pos; double fr2=pos-(double)k;
            pcm[j]=(k+1<sn)?(int)(mono[k]*(1.0-fr2)+mono[k+1]*fr2):mono[sn-1]; pos+=step; }
        free(mono); rate=MOTE_MIX_RATE; }
    /* optional non-destructive target override (parity with tools/mote): MOTE_SFX_RATE / MOTE_SFX_BITS */
    { const char *er=getenv("MOTE_SFX_RATE"), *eb=getenv("MOTE_SFX_BITS"); int tr=er?atoi(er):0, tb=eb?atoi(eb):0;
      if(tr>0 && (unsigned)tr<rate && N>1){ long M=(long)((long long)N*tr/(long long)rate); int *d2=malloc((size_t)(M>0?M:1)*sizeof(int));
          double step=(double)rate/tr,pos=0.0; for(long j=0;j<M;j++){ long k=(long)pos; double fr2=pos-(double)k; d2[j]=(k+1<N)?(int)(pcm[k]*(1.0-fr2)+pcm[k+1]*fr2):pcm[N-1]; pos+=step; }
          free(pcm); pcm=d2; N=M; rate=(unsigned)tr; }
      if(tb==8 && out_bits==16){ for(long j=0;j<N;j++){ int v=pcm[j]>>8; if(v>127)v=127; if(v<-128)v=-128; pcm[j]=v; } out_bits=8; } }
    const char *ctype = out_bits==8 ? "int8_t" : "int16_t";
    FILE *o=fopen(header,"w"); if(!o){ free(pcm); free(b); return -1; }
    fprintf(o,"/* GENERATED by Mote Studio (wav2snd) from %s. */\n#ifndef MOTE_SND_%s_H\n#define MOTE_SND_%s_H\n#include \"mote_api.h\"\n\n",path,name,name);
    fprintf(o,"static const %s %s_pcm[%ld] = {\n",ctype,name,N>0?N:1);
    for(long k=0;k<N;k++){ fprintf(o,"%d,",pcm[k]); if((k&15)==15)fputc('\n',o); } if(N<=0)fprintf(o,"0");
    fprintf(o,"\n};\nstatic const MoteSound %s_snd = { %s_pcm, %ld, %u, %u, 0 };\n\n#endif\n",name,name,N>0?N:1,rate,out_bits);
    fclose(o); free(pcm); free(b); { char m[180]; snprintf(m,sizeof m,"baked %s: %ld samples %ubit %uHz -> %s",name,N,out_bits,rate,header); log(m); } return 0; }
...
static void bake_dir(const char *adir, const char *srcdir,
    ...
    while((e=readdir(d))){ const char *n=e->d_name; if(n[0]=='.'||!strcmp(n,"build"))continue;
        ...
        else if(!strcasecmp(n+l-4,".stl")){ ensure_tool("stl2mesh.c",stltool,log); seen_claim(seen,base,n,log); char c[1100]; snprintf(c,sizeof c,"%s %s %s %s 1500 2>&1",stltool,base,path,header); if(run_logged(c,log)==0)(*did)++; }  /* colour at argv[5]; flags at argv[6]+ */
