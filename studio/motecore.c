...
static int bake_image(const char *path, const char *header, const char *name, mote_log_fn log){
    ...
    for(int i=0;i<w*h;i++){ int r=d[i*4],g=d[i*4+1],b=d[i*4+2],a=d[i*4+3]; unsigned v;
        if(a<128)v=0xF81F; else { v=((r>>3)<<11)|((g>>2)<<5)|(b>>3); if(v==0xF81F)v=0xF81E; }
        fprintf(f,"0x%04x,",v); if((i&15)==15)fputc('\n',f); }
    fprintf(f,"\n};\nstatic const MoteImage %s_img = { %s_px, %d, %d, 0x%04X };\n#define %s_W %d\n#define %s_H %d\n\n#endif\n",name,name,w,h,0xF81F,name,w,name,h);
    fclose(f); stbi_image_free(d); { char m[160]; snprintf(m,sizeof m,"baked %s: %dx%d -> %s",name,w,h,header); log(m); } return 0; }
