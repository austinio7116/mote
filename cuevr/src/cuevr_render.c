...
"            float tw   = t + 0.007 * (fbm2(vec2(t * 26.0, cos(ang) * 0.7)) - 0.5);\n"
"            float ph   = tw * 120.0 + (2.6 * cue_r / 0.016) * cos(ang)\n"
...
"                float e = smoothstep(hw + aa, hw - aa, d);\n"
"                c = mix(c, ebony, e);\n"
...
"                // A SHEEN, not a wash. This was 2.6, added flat across the\n"
"                // whole butt and modulated by the grain, so every cue in the\n"
"                // range came out pale and streaky and no timber colour\n"
"                // survived it — ebony, rosewood and a stained forearm all\n"
"                // rendered the same dirty white. Confirmed by zeroing it: the\n"
...
"                float e = smoothstep(hw + aa2, hw - aa2, db);\n"
"                vec3 pane = burr;\n"
"                if (u_cnvcol > 0.5) {\n"
