...
"        if (iscloth > 0.01) {\n"
"            // Project on whichever pair of axes the surface faces least, so a\n"
"            // vertical cushion face gets a real texture instead of a smear.\n"
"            vec3 an = abs(normalize(v_nrm));\n"
"            vec2 pq = (an.y > an.x && an.y > an.z) ? v_local.xz\n"
"                    : (an.x > an.z) ? v_local.zy : v_local.xy;\n"
"            float cn = texture(u_fur, vec3(pq / u_feltspan, 0.0)).r;\n"
"            float cn2 = texture(u_fur, vec3(pq / (u_feltspan * 4.5), 0.0)).r;\n"
"            // Stronger than on the bed. The cushions are a darker shade, so the\n"
"            // same relative modulation is a smaller absolute step and reads as\n"
"            // flat next to a bed that has a nap.\n"
"            tc *= 1.0 + iscloth * ((cn - 0.86) * 0.42 + (cn2 - 0.86) * 0.20);\n"
"        }\n"
...
"        // Amplitudes measured against the reference photograph rather than\n"
"        // chosen. High-passing both and comparing said mine had MORE total\n"
"        // contrast (6.7% of mean against 4.4%) but HALF the pixel-to-pixel\n"
"        // change (0.70 against 1.43) — the energy was sitting at too coarse a\n"
"        // scale, which is exactly what reads as a low-res dotty texture rather\n"
"        // than as grain. So: most of it in the finest octave, very little in\n"
"        // the coarse ones, and a fourth octave finer still.\n"
"        float n0 = texture(u_fur, vec3(q / (u_feltspan * 0.42), 0.0)).r;\n"
"        float n1 = texture(u_fur, vec3(q / u_feltspan, 0.0)).r;\n"
"        float n2 = texture(u_fur, vec3(q / (u_feltspan * 3.7), 0.0)).r;\n"
"        float n3 = texture(u_fur, vec3(q / (u_feltspan * 13.0), 0.0)).r;\n"
"        float nap = 1.0 + (n0 - 0.86) * 0.24\n"
"                        + (n1 - 0.86) * 0.17\n"
"                        + (n2 - 0.86) * 0.048\n"
"                        + (n3 - 0.86) * 0.026;\n"
...
"        // Sheen, and it has to be MEAN. Measured against the reference the\n"
"        // cloth was coming out 22% too bright and visibly desaturated, and\n"
"        // this was why: a white grazing term smeared over the whole bed washes\n"
"        // the green out of it. All the while I was chasing noise statistics,\n"
"        // which were already close — the visible difference was colour.\n"
"        float sheen = pow(1.0 - abs(dot(nv, V)), 5.0) * (0.012 + 0.030 * n1);\n"
"\n"
...
"        o_col = emit(c * (0.22 + 0.76 * ndl) + vec3(sheen * (1.0 - m * 0.85)),\n"
"                     1.0, 1.0);\n"
"    } else if (u_mode == 9) {\n"
...
static void bake_fur(void) {
    ...
    const int PA = 44, PC = 64;         /* periods along / across the tile */
