...
"        // A BALL IS NOT A LAMP. The ambient floor was 0.52 — over half the\n"
"        // albedo before any light reached it — so nothing on a ball ever got\n"
"        // dark and the whole set read as self-luminous plastic. 0.26 leaves a\n"
"        // real shadow side and the balls take on form.\n"
"        //\n"
"        // The two terms still SUM to 1.14, so a fragment facing the lamp is\n"
"        // lit exactly as brightly as before. Only the shadow side moves. The\n"
"        // complaint was that the balls glowed, not that they were too bright.\n"
"        vec3 c = bc * (0.26 + 0.88 * diff) * occ\n"
...
"            }\n"
"        }\n"
"        // THE CLOTH, MIRRORED IN THE UNDERSIDE. A ball is polished, and by\n"
"        // far the largest thing near it is two metres of coloured wool a\n"
"        // centimetre below — so the bottom of every ball carries a wash of\n"
"        // the cloth, and the white carries it most. There is already a\n"
"        // DIFFUSE bounce above, which is a different thing: that fills the\n"
"        // shadow under the ball, this is the bed actually reflected in it.\n"
"        //\n"
"        // Cheap on purpose: the reflected ray is already to hand for the\n"
"        // lamps, so this is a clamp, a dot, a pow and a multiply-add. No\n"
"        // second pass, no probe, no loop. Where the reflected ray points\n"
"        // DOWN it can only be seeing the bed, and how strongly is Fresnel —\n"
"        // the glancing lower rim of a ball is nearly a mirror, which is\n"
"        // exactly where the green is strongest in a photograph.\n"
"        {\n"
"            float dn   = clamp(-Rv.y, 0.0, 1.0);\n"
"            float fres = pow(1.0 - clamp(dot(nw, V), 0.0, 1.0), 4.0);\n"
"            c += u_clothsh * dn * (0.22 + 0.85 * fres);\n"
"        }\n"
...
"        float cue_r = 0.0051 + clamp((t*1.45 - 0.032)/1.378, 0.0, 1.0) * 0.0109;\n""        /* DERIVATIVES LIVE HERE, in uniform control flow. fwidth() inside a\n"
"         * varying branch is undefined on the branch boundary — the row of\n"
"         * pixels entering \'if (t > ms_tip)\' computed garbage derivatives and\n"
"         * the AA maths drew them as a dark dashed ring exactly on the line.\n"
"         * Every screen-space width is computed once, unconditionally. */\n"
"        float f0h = fract(a * max(u_cpts, 1.0));\n"
"        float d0h = min(f0h, 1.0 - f0h);\n"
"        float fw_d  = fwidth(d0h) * 1.2;\n"
"        float db_h  = abs(fract(a + u_cface) - 0.5) * 4.0;\n"
"        float fw_db = fwidth(db_h) * 1.2;\n"
"        float fw_t  = fwidth(t) * 1.2;\n"
"        float butt_varn = 0.0;\n"
"        vec3 ash   = u_cshaft;\n"
...
"                /* 0.68, not 0.60: the OUTER stack veneers sit wln*2*vi beyond\n"
"                 * the prong edge and pinch closed correspondingly later — at\n"
"                 * 0.60 they never finished and the old hard cut at ms_base\n"
"                 * truncated them mid-flow, the classic abrupt stop. */\n"
"                float hw = 0.68 * (sqrt(k * k + kr * kr) - kr) / (sqrt(1.0 + kr * kr) - kr);\n"
...
"                 * in focus. */\n"
"                /* The AA can never rival the feature. At grazing angles\n"
"                 * fwidth(d) explodes, and an AA band comparable to hw smears\n"
"                 * each prong apex into a half-strength wash — four smears in\n"
"                 * a ring around the shaft, the dark dashed line that kept\n"
"                 * coming back. Clamp the band to a fraction of hw, and let a\n"
"                 * sub-pixel prong DIM by coverage rather than widen. */\n"
"                float pxp = fw_d;\n"
...
"                 * met and the veneers pinched — black arrives from geometry. */\n"
"                if (u_cpts < 0.5) c = ebony;\n"
...
"                    float lf = clamp(1.2 / max(fw_t * 40.0 + fw_d * 5.8, 1e-3) - 0.15, 0.0, 1.0);\n"
"                    bm = mix(0.55, bm, lf);\n"
...
"                float px2 = fw_db;\n"
"                float aa2 = px2;   /* re-derived per curve below */\n"
...
"                    float aao = min(px2, hwo * 0.45);\n"
"                    if (u_cflash > 0.5)\n"
"                        c = mix(c, u_caccent, smoothstep(hwo + aao, hwo - aao, db)\n"
"                                              * clamp(hwo / px2, 0.0, 1.0));\n"
"                    float aai = min(px2, hwi * 0.45);\n"
"                    c = mix(c, pane, smoothstep(hwi + aai, hwi - aai, db)\n"
"                                     * clamp(hwi / px2, 0.0, 1.0));\n"
"                }\n""                if (u_cpflip > 0.5) {\n"
...
"                        float cr  = 1.0 - smoothstep(0.009, 0.009 + fw_t, abs(t - tcr));\n"
