...
"        \n"
"        if (iscloth < 0.99) {\n"
"            // Timber, MUCH quieter than it was and in two fetches rather than three.\n"
"            // The heavy figure read well in a still and was awful in a headset: at that\n"
"            // amplitude it is a leopard print you cannot stop looking at, and a rail is\n"
"            // meant to be furniture you glance past. Fine lines along the length, low\n"
"            // contrast, and a slow variation so it is not a barcode.\n"
"            vec3 an = abs(normalize(v_nrm));\n"
"            vec2 wq = (an.x > an.z) ? vec2(v_local.z, v_local.y * 0.08 + v_local.x * 0.02)\n"
"                                    : vec2(v_local.x, v_local.y * 0.08 + v_local.z * 0.02);\n"
"            float w1 = texture(u_nap, wq / vec2(0.090, 0.0045)).r - 0.5;\n"
"            float w2 = texture(u_nap, wq / vec2(0.320, 0.019)).g - 0.5;\n"
"            float grain = 1.0 + w1 * 0.13 + w2 * 0.10;\n"
