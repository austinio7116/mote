...
"// Knots, and the grain that flows around them.\n"
"//\n"
"// A knot is where a branch left the trunk, so the rings do not just stop at it —\n"
"// they SWELL and sweep around it, and that deflection is what the eye actually reads\n"
"// as a knot. A dark ellipse painted on parallel bands reads as a stain. So this\n"
"// returns a bulge to be added to the pith distance BEFORE the rings are formed, and\n"
"// the dark core separately.\n"
"//\n"
"// One candidate per cell along the board, most of them empty, position and size from\n"
"// a hash so no two rails are alike. No texture fetches: it is all arithmetic.\n"
"float knots(vec2 wq, out float core) {\n"
"    // 12 cm cells and most of them occupied. At 30 cm and 45% occupancy a metre\n"
"    // of visible rail expected barely one knot, so the feature was in the shader\n"
"    // and invisible in every screenshot of it.\n"
...
"    for (int i = -1; i <= 1; i++) {\n"
"        float cx = ci + float(i);\n"
"        if (hash12(vec2(cx, 3.7)) < 0.28) continue;      /* some cells are clear */\n"
"        vec2 kp = vec2((cx + 0.15 + 0.7 * hash12(vec2(cx, 11.0))) * CELL,\n"
"                       (hash12(vec2(cx, 29.0)) - 0.5) * 0.070);\n"
"        float kr = 0.011 + 0.019 * hash12(vec2(cx, 17.0));\n"
"        vec2 dv = (wq - kp) / vec2(1.0, 0.55);           /* knots are elliptical */\n"
"        float r = length(dv);\n"
"        bulge += exp(-(r * r) / (kr * kr * 2.6)) * kr * 6.5;\n"
"        core = max(core, 1.0 - smoothstep(kr * 0.50, kr * 0.92, r));\n"
"    }\n"
"    return bulge;\n"
"}\n"
"\n"
...
"    float kcore;\n"
"    float kbulge = knots(wq, kcore);\n"
"    float d = length(vec2(across + warp.x * RING * 1.6 + kbulge, depth));\n"
...
"    vec3 kc = base * 0.30;\n"
"    kc.r *= 1.25; kc.g *= 1.02; kc.b *= 0.72;\n"
"    c = mix(c, kc, kcore * 0.90);\n"
"    c *= 1.0 - pore * 0.14;\n"
...
"            // Board axes, NOT surf_uv. surf_uv derives its tangent from the\n"
"            // normal, so on a horizontal rail top it hands back (z, x) — the\n"
"            // \"along the board\" axis came out ACROSS the table. Knot cells were\n"
"            // then stepping across a 60 mm width instead of along a 2 m rail, and\n"
"            // the pith distance was being formed from a span of metres, so the\n"
"            // rings washed out and the knots never appeared anywhere except the\n"
"            // pocket throat, where the axes happened to fall out differently.\n"
"            //\n"
"            // A rail runs along whichever world axis it is long in: the side rails\n"
"            // along x, the end rails along z. For a vertical face the board axis is\n"
"            // its horizontal tangent.\n"
"            vec2 along2;\n"
"            {\n"
"                vec3 nn = normalize(v_nrm);\n"
"                if (abs(nn.y) > 0.7)\n"
"                    along2 = (abs(v_local.z) > abs(v_local.x)) ? vec2(1.0, 0.0)\n"
"                                                              : vec2(0.0, 1.0);\n"
"                else\n"
"                    along2 = normalize(vec2(-nn.z, nn.x) + 1e-6);\n"
"            }\n"
"            vec2 wq = vec2(dot(v_local.xz, along2),\n"
"                           dot(v_local.xz, vec2(-along2.y, along2.x)));\n"
